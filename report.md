...
1. 更新项目 `README.md`，补充中文项目说明和开发流程。
2. 搭建完整的项目目录结构。
3. 实现最小可运行的数据预处理脚本。
4. 实现最小可运行的条件扩散模型训练脚本。
5. 实现采样脚本，可以从 checkpoint 生成图像。
6. 实现简单评估脚本，用于 sanity check。
7. 完成一次端到端 smoke test，验证数据处理、训练、采样、评估链路全部可执行。
8. 将上述工作提交并推送到 GitHub 主分支。
